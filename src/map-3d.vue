...
import '../../../assets/js/echarts/echarts-gl@2.0.8.min'
  ...
  series: [
    {
      ...
      barSize: 1, // 柱子粗细
      shading: 'realistic', // realistic lambert color 三维柱状图中三维图形的着色效果。
      bevelSize: 1, // 柱子的倒角尺寸。支持设置为从 0 到 1 的值。
...
export default {
  ...
  methods: {
    ...
    createMap() {
      ...
      const seriesBarData = convertData(data)
      function setSeriesBarData() {
        let timer = null, count = 0, barData = []
        timer = setInterval(() => {
          if (count > data.length - 1) {
            ...
            barData.push(seriesBarData[count])
            count = count + 1
            echart.setOption({
              series: [
                {
